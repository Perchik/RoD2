...
def main():
    ...
    photos.create(('path',str),('timestamp',str),('aestheticscore',int),('location',locations), ('event',events),mode="override")
    ...
    faces.create(('photo',photos),('thumbnail',str),('person',people),('pid',int),('x',int),('y',int),('w',int),('h',int),mode="override")
    ...
    for i in range(len(photolist)):
    	photos.insert(photolist[i],eventLabels[i][1], 0,locations(name=geotaglist[i][1])[0],events(name=eventLabels[i][0])[0])
